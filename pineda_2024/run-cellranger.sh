...
PROJDIR="/data/CARD_MPU/data/pineda_2024"
cd ${PROJDIR}
...
SID=$(sed -n ${N}p samples.txt | awk '{print $1}')
SRR=$(sed -n ${N}p samples.txt | awk '{print $2}')
...
dirs=($(find FASTQS -maxdepth 3 -type f -name ${SRR}*.fastq.gz -exec dirname {} \; | sort -u))
for i in ${dirs[@]}; do
    j=$(realpath $i)
    MYCMD+=( "--fastqs $j" )
done
